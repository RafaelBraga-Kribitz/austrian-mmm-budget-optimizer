...
| [ADR-010](ADR-010_md073-rung4-fix-s.md) | MD-073 rung 4: fix Hill slope `s_c = 1` | **Superseded** by ADR-011 | Attempted after rungs 1–3 left 2 divergences. Did not reach 0; constant `s` NaN-R-hat. Free `s` restored. |
| [ADR-011](ADR-011_md073-rung1-target-accept-099.md) | Restore free `s`; extend rung 1 to `target_accept` 0.99 | **Ratified** | ADR-010 rolled back. After 0.95 still divergences-only, one retry at 0.99. Settings.sampler unchanged. |
